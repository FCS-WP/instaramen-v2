...
if (!class_exists('Veres_Ajax_Manager')) {
    ...
    class Veres_Ajax_Manager
    {
	    ...
	    public function handle_ajax_request() {
		    if ( ! $this->verify_request_nonce() ) {
			    ...
			    $this->add_response_data( false, esc_html__( 'Token Expired.', 'veres' ) )
			         ->send_error( 401 );
		    }

		    if(empty($_REQUEST['actions'])){
			    $this->add_response_data( false, esc_html__( 'Action not found.', 'veres' ) )
			         ->send_error( 401 );
		    }
		    ...
		    foreach ( $this->requests as $id => $action_data ) {
			    ...
			    if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				    $this->add_response_data( false, esc_html__( 'Action not found.', 'veres' ), 400 );
	    ...
	    private function send_success() {
		    ...
		    if ( function_exists( 'gzencode' ) ) {
			    ...
			    echo veres_render_variable($response); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		    } else {
			    echo veres_render_variable($json); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		    }
	     ...
	     *
	     * @return Veres_Ajax_Manager An instance of ajax manager.
	     */
