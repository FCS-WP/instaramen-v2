...
if(!defined('VERES_THEME_VERSION')){
    define('VERES_THEME_VERSION', '1.0.5');
}

if(!class_exists('Veres_Theme_Class')){

    final class Veres_Theme_Class {
	    ...
	    /**
	     * @var Veres_Ajax_Manager $ajax_manager;
	     */
	    public $ajax_manager;
        ...
        public function __construct() {
			...
			$this->ajax_manager = new Veres_Ajax_Manager();
        ...
        public function theme_setup() {
            ...
            $ext = apply_filters('veres/use_minify_css_file', false) || ( defined('WP_DEBUG') && WP_DEBUG ) ? '' : '.min';

            // Load text domain
            load_theme_textdomain( 'veres', self::$template_dir_path .'/languages' );
            ...
            register_nav_menus( array(
                'main-nav'   => esc_attr_x( 'Main Navigation', 'admin-view', 'veres' )
            ) );
             ...
             */
            add_theme_support( 'custom-header', apply_filters( 'veres/filter/custom_header_args', array(
                'width'              => 2000,
                'height'             => 1200,
                'flex-height'        => true,
                'video'              => true,
            ...
            if( veres_string_to_bool( veres_get_theme_mod('woocommerce_gallery_zoom') ) ){
                add_theme_support( 'wc-product-gallery-zoom');
            }
            if( veres_string_to_bool( veres_get_theme_mod('woocommerce_gallery_lightbox') ) ){
            ...
            add_theme_support( 'editor-color-palette', array(
                ...
                array(
                    'name' => esc_attr_x( 'pale pink', 'admin-view', 'veres' ),
                    'slug' => 'pale-pink',
                    'color' => '#f78da7',
                ),

                array(
                    'name' => esc_attr_x( 'theme primary', 'admin-view', 'veres' ),
                    'slug' => 'veres-theme-primary',
                    'color' => '#BC8157',
                ),

                array(
                    'name' => esc_attr_x( 'theme secondary', 'admin-view', 'veres' ),
                    'slug' => 'veres-theme-secondary',
                    'color' => '#212121',
                ),

                array(
                    'name' => esc_attr_x( 'strong magenta', 'admin-view', 'veres' ),
                    'slug' => 'strong-magenta',
                    'color' => '#a156b4',
                ),
                array(
                    'name' => esc_attr_x( 'light grayish magenta', 'admin-view', 'veres' ),
                    'slug' => 'light-grayish-magenta',
                    'color' => '#d0a5db',
                ),
                array(
                    'name' => esc_attr_x( 'very light gray', 'admin-view',  'veres' ),
                    'slug' => 'very-light-gray',
                    'color' => '#eee',
                ),
                array(
                    'name' => esc_attr_x( 'very dark gray', 'admin-view', 'veres' ),
                    'slug' => 'very-dark-gray',
                    'color' => '#444',
                ),
            ) );
        ...
        public function theme_setup_default(){
            $check_theme = get_option('veres_has_init', false);
            if(!$check_theme || !get_option('lastudio-kit-settings')){
                ...
                update_option('veres_has_init', true);
        ...
        public function meta_viewport() {
            ...
            echo apply_filters( 'veres_meta_viewport', $viewport );
        ...
        public function admin_scripts() {
            ...
            wp_enqueue_style( 'veres-font-lastudioicon', get_theme_file_uri( '/assets/css/lastudioicon.min.css' ), false, '1.0.0' );
            wp_enqueue_style( 'veres-google-fonts', $this->enqueue_google_fonts_url() , array(), null );
            wp_enqueue_style( 'veres-typekit-fonts', $this->enqueue_typekit_fonts_url() , array(), null );
        }
        ...
        public function theme_css() {
            ...
            // Load font icon style
            wp_enqueue_style( 'veres-font-lastudioicon', get_theme_file_uri ('/assets/css/lastudioicon'.$ext.'.css' ), false, $theme_version );
            ...
            wp_enqueue_style( 'veres-theme', get_parent_theme_file_uri('/style'.$ext.'.css'), false, $theme_version );

            $this->render_extra_style();

            $additional_inline_stype = veres_minimizeCSS($this->extra_style);

            $inline_handler_name = 'veres-theme';

            if(function_exists('WC')){
                wp_enqueue_style( 'veres-woocommerce', get_theme_file_uri( '/assets/css/woocommerce'.$ext.'.css' ), false, $theme_version );
                $inline_handler_name = 'veres-woocommerce';
            }
        ...
        public function theme_js() {
            ...
            $ext = !apply_filters('veres/use_minify_js_file', true) || ( defined('WP_DEBUG') && WP_DEBUG ) ? '' : '.min';
            ...
            if(function_exists('WC')){
                $dependencies[] = 'veres-woocommerce';
            }

            $dependencies = apply_filters('veres/filter/js_dependencies', $dependencies);

            wp_enqueue_script('veres-theme', get_theme_file_uri( '/assets/js/app'.$ext.'.js' ), $dependencies, $theme_version, true);

            if (is_singular() && comments_open() && get_option('thread_comments')) {
                wp_enqueue_script('comment-reply');
            }

            if(apply_filters('veres/filter/force_enqueue_js_external', true)){
                wp_localize_script('veres-theme', 'la_theme_config', $localize_array );
            }

            if(function_exists('la_get_polyfill_inline')){

                $polyfill_data = apply_filters('veres/filter/js_polyfill_data', [
                    'veres-polyfill-object-assign' => [
                        'condition' => '\'function\'==typeof Object.assign',
                        'src'       => get_theme_file_uri( '/assets/js/lib/polyfill-object-assign'.$ext.'.js' ),
                        'version'   => $theme_version,
                    ],
                    'veres-polyfill-css-vars' => [
                ...
                if(!empty($polyfill_inline)){
                    wp_add_inline_script('veres-theme', $polyfill_inline, 'before');
                }
            }
        ...
        public function localize_array() {
            ...
            $template_cache = veres_string_to_bool(veres_get_option('template_cache'));
            ...
            $array = array(
                ...
                'single_ajax_add_cart' => veres_string_to_bool( veres_get_theme_mod('single_ajax_add_cart') ),
                'i18n' => array(
                    'backtext' => esc_attr_x('Back', 'front-view', 'veres'),
                    'compare' => array(
                        'view' => esc_attr_x('View List Compare', 'front-view', 'veres'),
                        'success' => esc_attr_x('has been added to comparison list.', 'front-view', 'veres'),
                        'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'veres')
                    ),
                    'wishlist' => array(
                        'view' => esc_attr_x('View List Wishlist', 'front-view', 'veres'),
                        'success' => esc_attr_x('has been added to your wishlist.', 'front-view', 'veres'),
                        'error' => esc_attr_x('An error occurred, Please try again !', 'front-view', 'veres')
                    ),
                    'addcart' => array(
                        'view' => esc_attr_x('View Cart', 'front-view', 'veres'),
                        'success' => esc_attr_x('has been added to your cart', 'front-view', 'veres'),
                        'error' => esc_attr_x('An error occurred, Please try again !', 'front-view', 'veres')
                    ),
                    'global' => array(
                        'error' => esc_attr_x('An error occurred ,Please try again !', 'front-view', 'veres'),
                        'search_not_found' => esc_attr_x('It seems we can’t find what you’re looking for, please try again !', 'front-view', 'veres'),
                        'comment_author' => esc_attr_x('Please enter Name !', 'front-view', 'veres'),
                        'comment_email' => esc_attr_x('Please enter Email Address !', 'front-view', 'veres'),
                        'comment_rating' => esc_attr_x('Please select a rating !', 'front-view', 'veres'),
                        'comment_content' => esc_attr_x('Please enter Comment !', 'front-view', 'veres'),
                        'continue_shopping' => esc_attr_x('Continue Shopping', 'front-view', 'veres'),
                        'cookie_disabled' => esc_attr_x('We are sorry, but this feature is available only if cookies are enabled on your browser', 'front-view', 'veres'),
                        'more_menu' => esc_attr_x('Show More +', 'front-view', 'veres'),
                        'less_menu' => esc_attr_x('Show Less', 'front-view', 'veres'),
                        'search_view_more' => esc_attr_x('View More', 'front-view', 'veres'),
                    )
                ),
                'popup' => array(
                    'max_width' => esc_attr(veres_get_option('popup_max_width', 790)),
                    'max_height' => esc_attr(veres_get_option('popup_max_height', 430))
                ),
                'js_path'       => esc_attr(apply_filters('veres/filter/js_path', self::$template_dir_url . '/assets/js/lib/')),
                'js_min'        => apply_filters('veres/use_minify_js_file', true),
                'theme_path'    => esc_attr(apply_filters('veres/filter/theme_path', self::$template_dir_url . '/')),
                ...
                'cache_ttl' => apply_filters('veres/cache_time_to_life', !$template_cache ? 30 : (60 * 5)),
                'local_ttl' => apply_filters('veres/local_cache_time_to_life', !$template_cache ? 30 : (60 * 60 * 24)),
            ...
            return apply_filters( 'veres/filter/localize_array', $array );
        }
        ...
        public function register_sidebars() {
            ...
            $heading = 'div';
            $heading = apply_filters( 'veres/filter/sidebar_heading', $heading );

            // Default Sidebar
            register_sidebar( array(
                'name'			=> esc_html__( 'Default Sidebar', 'veres' ),
                'id'			=> 'sidebar',
                'description'	=> esc_html__( 'Widgets in this area will be displayed in the left or right sidebar area if you choose the Left or Right Sidebar layout.', 'veres' ),
        ...
        public static function enqueue_google_fonts_url(){
            ...
            if ( 'off' !== _x( 'on', 'Archivo font: on or off', 'veres' ) ) {
                $fonts[] = 'Archivo:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;1,300;1,400;1,500;1,600;1,700;1,800';
            }
        ...
        public static function enqueue_typekit_fonts_url(){
	        ...
	        if ( 'off' !== _x( 'on', 'Orpheus Pro font: on or off', 'veres' ) ) {
		        $fonts_url = '//use.typekit.net/pyg3oji.css';
	        }
        ...
        public function register_breakpoint(){
            if(defined('ELEMENTOR_VERSION') && class_exists('Elementor\Core\Breakpoints\Manager', false)){
                $has_register_breakpoint = get_option('veres_has_register_breakpoint', false);
                if(empty($has_register_breakpoint)){
                    ...
                    $default_settings = [
                        ...
                        'system_colors' => [
	                        [
		                        '_id' => 'primary',
		                        'title' => esc_html__( 'Primary', 'veres' )
	                        ],
	                        [
		                        '_id' => 'secondary',
		                        'title' => esc_html__( 'Secondary', 'veres' )
	                        ],
	                        [
		                        '_id' => 'text',
		                        'title' => esc_html__( 'Text', 'veres' )
	                        ],
	                        [
		                        '_id' => 'accent',
		                        'title' => esc_html__( 'Accent', 'veres' )
	                        ]
                        ],
                        'system_typography' => [
	                        [
                    ...
                    update_option('veres_has_register_breakpoint', true);
                }
            }
        }
    }

    Veres_Theme_Class::get_instance();
