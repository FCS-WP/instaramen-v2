...
if ( ! function_exists( 'veres_get_theme_mod' ) ) {

	function veres_get_theme_mod( $key = '', $default = '' ) {
		$mods = get_theme_mods();
		...
		return apply_filters( 'veres/filter/veres_get_theme_mode', $value, $key, $default, $mods );
	}

}

if ( ! function_exists( 'veres_get_option' ) ) {

	function veres_get_option( $key = '', $default = '' ) {
		$theme_options = get_option( 'veres_options', array() );
		...
		return apply_filters( 'veres/filter/get_option', $value, $key, $default, $theme_options );
	}
...
if ( ! function_exists( 'veres_render_variable' ) ) {
	function veres_render_variable( $variable ) {
		return $variable;
	}
}
 ...
 * @return string
 */
if ( !function_exists('veres_render_style_attribute') ){
    function veres_render_style_attribute( $inline_css ) {
        if(!empty($inline_css)){
...
if ( ! function_exists( 'veres_transfer_text_to_format' ) ) {
	function veres_transfer_text_to_format( $content, $autop = false ) {
		if ( $autop ) {
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'veres_comment' ) ) {

	function veres_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
                ...
                <div id="comment-<?php comment_ID(); ?>" class="comment-container">
                    <p><?php esc_html_e( 'Pingback:', 'veres' ); ?>
                        <span><span><?php comment_author_link(); ?></span></span> <?php edit_comment_link( esc_html__( '(Edit)', 'veres' ), '<span class="edit-link">', '</span>' ); ?>
                    </p>
                </div>
            ...
            <div id="comment-<?php comment_ID(); ?>" class="comment-container">
                ...
                <div <?php comment_class( 'comment-body' ); ?>>
					...
					<?php echo get_avatar( $comment, apply_filters( 'veres_comment_avatar_size', 150 ) ); ?>

                    <div class="comment-content-outer">

                        <div class="comment-author">
                            <h3
                              class="comment-link"><?php printf( esc_html__( '%s ', 'veres' ), sprintf( '%s', get_comment_author_link() ) ); ?></h3>
                        ...
                        <div class="comment-entry">
							<?php if ( '0' == $comment->comment_approved ) : ?>
                                <p
                                  class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'veres' ); ?></p>
							<?php endif; ?>
                        ...
                        <span class="comment-meta commentmetadata">
                            ...
                            <?php edit_comment_link( __( 'edit', 'veres' ) ); ?>
                        </span>
                    </div>
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'veres_modify_comment_form_fields' ) ) {

	function veres_modify_comment_form_fields( $fields ) {

		$commenter = wp_get_current_commenter();
		$req       = get_option( 'require_name_email' );

		$fields['author'] = '<div class="comment-form-author"><input type="text" name="author" id="author" value="' . esc_attr( $commenter['comment_author'] ) . '" placeholder="' . esc_attr__( 'Name (required)', 'veres' ) . '" size="22" tabindex="101"' . ( $req ? ' aria-required="true"' : '' ) . ' class="input-name" /></div>';

		$fields['email'] = '<div class="comment-form-email"><input type="text" name="email" id="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" placeholder="' . esc_attr__( 'Email', 'veres' ) . '" size="22" tabindex="102"' . ( $req ? ' aria-required="true"' : '' ) . ' class="input-email" /></div>';

		$fields['url'] = '<div class="comment-form-url"><input type="text" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="' . esc_attr__( 'Website', 'veres' ) . '" size="22" tabindex="103" class="input-website" /></div>';

		return $fields;

	}

	add_filter( 'comment_form_default_fields', 'veres_modify_comment_form_fields' );
 ...
 * String to boolean
 */
if ( ! function_exists( 'veres_string_to_bool' ) ) {
	function veres_string_to_bool( $string ) {
 ...
 */
if ( ! function_exists( 'veres_get_post_thumbnail_format' ) ) {
	function veres_get_post_thumbnail_format( $thumbnail_size = 'full', $post_link = true ) {
			...
			case 'quote':
				...
				if ( ! empty( $_quote_text ) ) {
					...
					if(has_post_thumbnail()){
						$format_output = sprintf('<div class="postformat-content postformat-content--standard postformat-content--quote" %3$s><div class="postformat-content--inner">%4$s</div><div class="postformat-content--quote-inner"><div class="postformat-content-text">%1$s</div>%2$s</div></div>', esc_html( $_quote_text ), $_quote_cite, veres_render_style_attribute($_quote_style), get_the_post_thumbnail( $post_id, $thumbnail_size ) );
                    }
					else{
						$format_output = sprintf( '<div class="postformat-content postformat-content--quote" %3$s><div class="postformat-content--quote-inner"><div class="postformat-content-text">%1$s</div>%2$s</div></div>', esc_html( $_quote_text ), $_quote_cite, veres_render_style_attribute($_quote_style) );
                    }
...
if ( ! function_exists( 'veres_entry_meta_item_category_list' ) ) {
	function veres_entry_meta_item_category_list( $before = '', $after = '', $separator = ', ', $parents = '', $post_id = false ) {
		add_filter( 'get_the_terms', 'veres_exclude_demo_term_in_category' );
		$categories_list = get_the_category_list( '{{_}}', $parents, $post_id );
		veres_deactive_filter( 'get_the_terms', 'veres_exclude_demo_term_in_category' );
		if ( $categories_list ) {
			printf(
				'%3$s<span class="screen-reader-text">%1$s </span><span>%2$s</span>%4$s',
				esc_html_x( 'Posted in', 'front-view', 'veres' ),
...
if ( ! function_exists( 'veres_exclude_demo_term_in_category' ) ) {
	function veres_exclude_demo_term_in_category( $term ) {
		return apply_filters( 'veres/post_category_excluded', $term );
	}
}

if ( ! function_exists( 'veres_deactive_filter' ) ) {
	function veres_deactive_filter( $tag, $function_to_remove, $priority = 10 ) {
		return call_user_func( 'remove_filter', $tag, $function_to_remove, $priority );
	}
}

if ( ! function_exists( 'veres_wpml_object_id' ) ) {
	function veres_wpml_object_id( $element_id, $element_type = 'post', $return_original_if_missing = false, $ulanguage_code = null ) {
...
if ( ! function_exists( 'veres_is_blog' ) ) {
	function veres_is_blog() {
		return ( is_home() || is_tag() || is_category() || is_date() || is_year() || is_month() || is_author() ) ? true : false;
	}
}

if ( ! function_exists( 'veres_get_wishlist_url' ) ) {
	function veres_get_wishlist_url() {
		$wishlist_page_id = veres_get_theme_mod( 'wishlist_page', 0 );

		return ( ! empty( $wishlist_page_id ) ? get_the_permalink( $wishlist_page_id ) : esc_url( home_url( '/wishlist/' ) ) );
	}
}

if ( ! function_exists( 'veres_get_compare_url' ) ) {
	function veres_get_compare_url() {
		$compare_page_id = veres_get_theme_mod( 'compare_page', 0 );

		return ( ! empty( $compare_page_id ) ? get_the_permalink( $compare_page_id ) : esc_url( home_url( '/compare/' ) ) );
	}
}

if( !function_exists('veres_minimizeCSS') ) {
    function veres_minimizeCSS( $css ){
