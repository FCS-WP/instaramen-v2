...
(function ($) {
    ...
    LaStudio.core.Blog = function ($sidebar_inner) {
        ...
        $('.widget > .widget-title', $sidebar_inner).each(function (){
            $(this).closest('.widget').addClass('allow-toggle-collapse');
        });
        $('.allow-toggle-collapse > .widget-title', $sidebar_inner).on('click', function (e){
            e.preventDefault();

            var $parent = $(this).closest('.widget'),
                $siblings = $(this).siblings('div,ul,form');

            if($parent.hasClass('w-collapsed')){
                $siblings.stop().slideDown(300, function (){
                    $parent.removeClass('w-collapsed')
                });
            }
            else{
                $siblings.stop().slideUp(300, function (){
                    $parent.addClass('w-collapsed');
                });
            }
        })
    };
    ...
    LaStudio.core.CustomFunction = function () {
        ...
        try {
            if (Cookies.get('veres_wishlist') !== 'undefined') {
                const _cW = JSON.parse(Cookies.get('veres_wishlist'));
                wishlist_compare_count.wishlist = _cW[0].posts.length
            }
            if (Cookies.get('veres_compare') !== 'undefined') {
                const _cC = JSON.parse(Cookies.get('veres_compare'));
        ...
        return {
            init: function () {
                if (waitfortrigger != 1) {
                    ...
                    try {
                        if (Cookies.get('veres_dont_display_popup') == 'yes') {
                            return;
                        }
                    } catch (ex) {
                        ...
                        setTimeout(function () {
                            LaStudio.core.OpenNewsletterPopup($popup, function () {
                                if ($('.cbo-dont-show-popup', $popup).length && $('.cbo-dont-show-popup', $popup).is(':checked')) {
                                    try {
                                        Cookies.set('veres_dont_display_popup', 'yes', {
                                            expires: backtime,
                                            path: '/'
                                        });
                                    } catch (ex) {
...
(function($) {
    ...
    function AutoDisplayPopup( $popup, callbackClose ){
    ...
    $(window).on('load', function (){
        ...
        $('.section-inline-popup[data-lakit-autopopup]').each(function (){
            var $popup = $(this),
                delay = parseInt($popup.data('lakit-autopopup-delay') || 2000),
                enable_cookie = (!!$popup.data('lakit-autopopup-cookie') && ($popup.data('lakit-autopopup-cookie') == 'yes' || $popup.data('lakit-autopopup-cookie') == 'true')) ? true : false,
                backtime = parseInt($popup.data('lakit-autopopup-cookie-backtime') || 1),
                cookieName = 'lakitpopup',
                checkcookie = false,
                disable_on_mobile = (!!$popup.data('lakit-autopopup-disablemobile') && ($popup.data('lakit-autopopup-disablemobile') == 'yes' || $popup.data('lakit-autopopup-disablemobile') == 'true')) ? true : false;
            if(enable_cookie && !!$popup.data('lakit-autopopup-cookie-name') && $popup.data('lakit-autopopup-cookie-name') != ''){
                cookieName = $popup.data('lakit-autopopup-cookie-name');
                checkcookie = true;
            }
            ...
            function callme(){
                ...
                setTimeout(function (){
                    AutoDisplayPopup($popup, function (){
                        if(checkcookie){
                            try {
